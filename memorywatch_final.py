...
def zz_feature_map_amplitudes(Za, n_qubits, reps=1):
    N = Za.shape[0]
    dim = 2 ** n_qubits
...
def three_way_comparison(X, y, cats, label, n_qubits=4, reps=1, seed=42, weighted=True):
...
def multi_seed_validation(seeds, n_polls, n_qubits=4, reps=1):
...
def three_way_comparison_silent(X, y, seed=42, n_qubits=4, reps=1, weighted=True):
...
def hparam_sweep(X, y, label, qubits_list, reps_list):
    print(f"\n{'='*72}\n  Qubit x reps sweep — {label}\n{'='*72}")
    print(f"  {'qubits':>7} {'reps':>5} {'expl.var':>9} {'RBF':>8} {'Quantum':>9} {'Δ':>8}")
    best = None
    for nq in qubits_list:
        for reps in reps_list:
            r = three_way_comparison_silent(X, y, n_qubits=nq, reps=reps)
            delta = r['qk_auc'] - r['rbf_auc']
            marker = "  <-- quantum wins" if delta > 0 else ""
            print(f"  {nq:>7} {reps:>5} {'':>9} {r['rbf_auc']:>8.4f} "
                  f"{r['qk_auc']:>9.4f} {delta:>+8.4f}{marker}")
            if best is None or r['qk_auc'] > best[2]['qk_auc']:
                best = (nq, reps, r)
    nq, reps, r = best
...
def three_way_on_real_data(X, y, cats, label, n_qubits=6, reps=2, seed=42,
...
def main():
    ...
    # 4. Three-way comparison on both datasets (default config: 4 qubits, reps=1)
    print("\n[step 4/8] Three-way comparison (default config: 4 qubits, reps=1)...")
    three_way_comparison(Xb, yb, catsb, "baseline dataset", n_qubits=4, reps=1)
    three_way_comparison(Xi, yi, catsi, "interaction-only dataset", n_qubits=4, reps=1)

    # 5. Multi-seed validation on baseline
    if not args.skip_multiseed:
        print("\n[step 5/8] Multi-seed validation (baseline dataset)...")
        multi_seed_validation(args.seeds, args.polls, n_qubits=4, reps=1)
    else:
    ...
    if not args.skip_sweep:
        ...
        hparam_sweep(Xb, yb, "baseline dataset", qubits_list=[4, 6, 8], reps_list=[1, 2])
        best_nq, best_reps = hparam_sweep(Xi, yi, "interaction-only dataset",
                                           qubits_list=[4, 6, 8], reps_list=[1, 2])
    else:
